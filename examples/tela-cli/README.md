...
### What is TELA-CLI?
TELA-CLI is an implementation of the `civilware/tela` [go package](https://pkg.go.dev/github.com/secretnamebasis/simple-tela) as a command line interface. With TELA-CLI, users can view and manage existing TELA content, along with creating and installing new TELA content.
...
```
git clone https://github.com/secretnamebasis/simple-tela.git
...
```
go install github.com/secretnamebasis/simple-tela/cmd/tela-cli@latest
tela-cli
```
...
#### Gnomon
[Gnomon](https://github.com/secretnamebasis/Gnomon) is the decentralized indexer used for querying and storing TELA smart contract data in a local DB. Gnomon indexer can be started using `gnomon start` and stopped with `gnomon stop`.
```
...
### TELA
More information on TELA Decentralized Web Standard and its components can be found in the `civilware/tela` [package repo](https://github.com/secretnamebasis/simple-tela).
