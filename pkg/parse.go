...
func parseDocShards(xswd_connection *websocket.Conn, sc dvm.SmartContract, path string) (docShards [][]byte, recreate, compression string, err error) {
	scids := parseINDEXForDOCs(sc)
	...
	for i, scid := range scids {
		...
		var fileName string
		if fileName, err = getContractVar(xswd_connection, scid, HEADER_NAME_V2.Trim()); err != nil {
		...
		if i == 0 {
			...
			if subDir, err := getContractVar(xswd_connection, scid, HEADER_SUBDIR.Trim()); err == nil && subDir != "" {
